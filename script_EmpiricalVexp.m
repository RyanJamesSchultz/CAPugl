% Script to perform CAP-tests on the data, using arbitrary Mmax vs V^a functions.
% Used to generate the data for Figures 14 & S16.
clear;
...
for k=1:length(D)
    ...
    % Loop over all exponent choices.
    for i=1:length(nv)

        % Fit the arbitrary Mmax model to the data.
        Afxn = @(c) min(Mmax_Vn(Vi,c,nv(i))-Mlrg)-dMm;
        c=fzero(Afxn,[1e-10 1e20]);
        Mmax_i=Mmax_Vn(Vi,c,nv(i));

        % Add arbitrary Mmax model as another hypothesis.
        Sm(i).Mmax=Mmax_i;
        Sm(i).c=c;
        Sm(i).n=nv(i);
        Sm(i).K=Kgr+1;
    ...
    % Add the unbound model.
    Sm(i+1).Mmax=(Inf)*ones(size(Mlrg));
    Sm(i+1).K=Kgr+0;
...
for i=1:length(W)-1
    plot([0,n]+1,[W(i).Mmax(1),W(i).Mmax],'-', 'DisplayName',['M_{MAX} V^{',num2str(nv(i)),'}']);
...
ax3=subplot(313);
for i=1:length(W)-1
    loglog([0,n]+1,OR(i,:),'-', 'DisplayName',['M_{MAX} V^{',num2str(nv(i)),'}']); hold on;
end
loglog(xlim(),1*[1 1],'-k');
loglog(xlim(),3*[1 1],':k');
...
% Simple function that will compute maximum magnitudes for arbitrary models.
function [Mmax]=Mmax_Vn(V,c,n)
  Mo=c*(V.^n);              % Seismic moment (N m).
